...
CSMC.AS.repM <- function(y,
  ...
  while (t <= T){
    ...
    ## pψ(x(bt)t|x(bt−1)t−1)
    log.px <- log(Px[particlesX[,t-1], X_ref[t]])
    
    ## f(y_t|theta_t^{(b_t)}, x_t^{(b_t)}) x gψ(θt|θ(bt−1)t−1,x(bt)t) x pψ(x(bt)t|x(bt−1)t−1)x w^(bt−1)_(t-1)
    log_w_as <- log(normalisedWeights[, t-1]) + log.fy + log.gtheta + log.px
