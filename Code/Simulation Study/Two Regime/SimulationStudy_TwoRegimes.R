...
pdf(paste0("../../../Figures/Simulation Study/Two Regime/TwoRegimeSimulatedData_seed", seed, "_T",T+1, ".pdf"), width = 12, height = 5)
par(mfrow=c(1,1))
plot(as.vector(y), 
     type = "p", 
     lty=1, 
...
pdf(paste0("../../../Figures/Simulation Study/Two Regime/TwoRegimeSimulatedSEIR_seed", seed, "_T",T+1, ".pdf"), width = 10, height = 7)
...
setwd("../../../Output/Simulation Study/Two Regime")
...
PG.results <- readRDS("../../../Output/Simulation Study/Two Regime/PG_results_pUnknown_seed1580_niter31000_M50_K2.rds")
PG.results <- readRDS("../../../Output/Simulation Study/Two Regime/PG_results_pUnknown_seed1580_niter31000_M50_K2.rds")

# Extract results from each chain 
MCMC.chain.1 <- PG.results[[1]][[1]]
MCMC.chain.2 <- PG.results[[1]][[2]]
...
# ### Try One Regime and get marginal likelihood
# regimes <- c(1)
# niter <- 1000
# 
# # Set up hyperparameters
...
# ptm <- proc.time() 
# PG.results.K1 <- PG.CSMC.AS(y, regimes, M, niter, hyperparams, pop.size=1, num.mh.updates=5)
# proc.time()-ptm 
# 
# 
# ### Try Three Regimes and get marginal likelihood
# regimes <- c(1,2,3)
# niter <- 1000
# 
# # Set up hyperparameters
...
# 
# # Load the script that contains the PG.CSMC.AS() function
# source("../Three Regime/ParticleGibbs_ThreeRegimes.R", local = TRUE)
# ptm <- proc.time() 
# PG.results.K3 <- PG.CSMC.AS(y, regimes, M, niter, hyperparams, pop.size=1, num.mh.updates=5)
# proc.time()-ptm 
# mean(PG.results.K3$marginalLogLik.CSMC.AS.repM)
# 
# 
# ### Try Four Regimes and get marginal likelihood
# regimes <- c(1,2,3,4)
# niter <- 1000
# 
# # Set up hyperparameters
...
# 
# # Load the script that contains the PG.CSMC.AS() function
# source("../../Real Data/Four Regime/ParticleGibbs_FourRegimes.R", local = TRUE)
# ptm <- proc.time() 
# PG.results.K4 <- PG.CSMC.AS(y, regimes, M, niter, hyperparams, pop.size=1, num.mh.updates=5)
# proc.time()-ptm 
# mean(PG.results.K4$marginalLogLik.CSMC.AS.repM)
# 
# 
# plot(1:niter, PG.results.K1$marginalLogLik.CSMC.AS.repM, type="l", 
#      ylab="Marginal Log-Likelihood", xlab = "Iterations")
# lines(MCMC.chain.1$marginalLogLik.CSMC.AS.repM[1:niter], col="blue")
# lines(PG.results.K3$marginalLogLik.CSMC.AS.repM[1:niter], col="red")
# lines(PG.results.K4$marginalLogLik.CSMC.AS.repM[1:niter], col="green")
# 
# # Add legend for K=1,2,3 and 4
# legend("bottomright", legend=c("K=1", "K=2", "K=3", "K=4"), 
...
setwd("../../../Figures/Simulation Study/Two Regime")

png(paste0(Sys.Date(), " SimulationTwoRegimeTracePlot_seed", seed, "_T", T+1, ".png"), 
    width = 6000, height = 5000, res=500)
par(mfrow=c(5,4)) 
par(mar = c(5, 5, 4, 2) + 0.2)  # Adjust the left margin as needed
options(scipen = 1000)

plot(MCMC.chain.1$parameters.CSMC.AS.repM$alpha[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(alpha), cex.lab=1.2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$alpha[1, burnin:niter], col="blue")
abline(h = alpha, col = "red")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$alpha[1, burnin:niter]), type="l", main="", xlab="", ylab = expression(alpha), panel.first=abline(v = alpha, col = "red"), cex.lab=1.2)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$alpha[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$beta[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(beta),  cex.lab=1.2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$beta[1, burnin:niter], col="blue")
abline(h = beta, col = "red")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$beta[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(beta), panel.first=abline(v = beta, col = "red"), cex.lab=1.2)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$beta[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$gamma[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(gamma), cex.lab=1.2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$gamma[1, burnin:niter], col="blue")
abline(h = gamma, col = "red")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$gamma[1, burnin:niter]), type="l",  main="",  xlab="", ylab = expression(gamma), panel.first=abline(v = gamma, col = "red"),  ylim=c(0,11), cex.lab=1.2)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$gamma[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$kappa[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(kappa),  ylim=c(1000, 12000), cex.lab=1.2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$kappa[1, burnin:niter], col="blue")
abline(h = kappa, col = "red")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$kappa[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(kappa), panel.first=abline(v = kappa, col = "red"), ylim=c(0, 0.0004), cex.lab=1.2)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$kappa[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$lambda[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(lambda), cex.lab=1.2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$lambda[1, burnin:niter], col="blue")
abline(h = lambda, col = "red")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$lambda[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(lambda), panel.first=abline(v = lambda, col = "red"), ylim=c(0, 0.0012), cex.lab=1.2)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$lambda[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$p[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(p), cex.lab=1.2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$p[1, burnin:niter], col="blue")
abline(h = p, col = "red")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$p[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(p), panel.first=abline(v = p, col = "red"), cex.lab=1.2)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$p[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$f[2, burnin:niter], type="l", xlab="Iterations", ylab = expression(f[2]), cex.lab=1.2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$f[2, burnin:niter], col="blue")
abline(h = f[2], col = "red")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$f[2, burnin:niter]), type="l",  main="", xlab="", ylab = expression(f[2]), panel.first=abline(v = f[2,1], col = "red"), ylim = c(0, 6), cex.lab=1.2)
...
plot(chain1.post.pi.k1[burnin:(niter-1),1],
     ...
     # ylim = c(0.6, 1),
     panel.first=abline(h = Px[1,1], col = "red"), cex.lab=1.2) # pi_11
lines(chain2.post.pi.k1[burnin:(niter-1),1], col="blue")
abline(h = Px[1,1], col = "red")
plot(density(chain1.post.pi.k1[burnin:(niter-1),1]),
     type="l",
     xlab="Iterations",
     ylab = expression(pi[11]),
     main= "",
     panel.first=abline(v = Px[1,1], col = "red"), cex.lab=1.2) # pi_11
lines(density(chain2.post.pi.k1[burnin:(niter-1),1]), col="blue")
...
plot(chain1.post.pi.k2[burnin:(niter-1),2],
     ...
     panel.first=abline(h = Px[2,2], col = "red"), ylim=c(0,1), cex.lab=1.2)
lines(chain2.post.pi.k2[burnin:(niter-1),2], col="blue")
abline(h = Px[2,2], col = "red")
plot(density(chain1.post.pi.k2[burnin:(niter-1),2]),
     type="l",
     xlab="Iterations",
     ylab = expression(pi[22]),
     main = "",
     panel.first=abline(v = Px[2,2], col = "red"), cex.lab=1.2)
lines(density(chain2.post.pi.k2[burnin:(niter-1),2]), col="blue")
...
niter <- 31000   # Update with your total number of iterations
...
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$alpha[1, burnin:niter]), type="l", main="", xlab="", ylab = expression(alpha), panel.first=abline(v = alpha, col = "red"), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$alpha[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$beta[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(beta), panel.first=abline(v = beta, col = "red"), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$beta[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$gamma[1, burnin:niter]), type="l",  main="",  xlab="", ylab = expression(gamma), panel.first=abline(v = gamma, col = "red"), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$gamma[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$kappa[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(kappa), panel.first=abline(v = kappa, col = "red"),cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$kappa[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$lambda[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(lambda), panel.first=abline(v = lambda, col = "red"), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$lambda[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$p[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(p), panel.first=abline(v = p, col = "red"), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$p[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$f[2, burnin:niter]), type="l",  main="", xlab="", ylab = expression(f[2]), panel.first=abline(v = f[2,1], col = "red"), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$f[2, burnin:niter]), col="blue")
...
plot(density(chain1.post.pi.k1[burnin:(niter-1),1]),
     ...
     ylab = expression(pi[11]),
     main= "",
     panel.first=abline(v = Px[1,1], col = "red"), cex.lab=1.5) # pi_11
lines(density(chain2.post.pi.k1[burnin:(niter-1),1]), col="blue")
# plot(post.pi.k1[1:(niter-1),2],
#      type="l",
#      xlab="Iterations",
...
plot(density(chain1.post.pi.k2[burnin:(niter-1),2]),
     ...
     ylab = expression(pi[22]),
     main = "",
     panel.first=abline(v = Px[2,2], col = "red"), cex.lab=1.5)
lines(density(chain2.post.pi.k2[burnin:(niter-1),2]), col="blue")



# Histogram of parameters
png(paste0(Sys.Date(), " SimulationTwoRegimeHistogram_seed", seed, "_T", T+1, ".png"), 
    width = 4000, height = 3000, res = 500)
par(mfrow=c(3,3))
...
# pi_11
hist(
  chain1.post.pi.k1[burnin:(niter-1),1],
...
abline(v = mean(chain1.post.pi.k1[burnin:(niter-1),1]), col = blue, lwd = 2)
pi11.CI <- quantile(chain1.post.pi.k1[burnin:(niter-1),1], c(0.025, 0.975))
abline(v = pi11.CI, col = blue, lty = 2, lwd = 2)


# pi_22
hist(
  chain1.post.pi.k2[burnin:(niter-1),2],
...
abline(v = mean(chain1.post.pi.k2[burnin:(niter-1),2]), col = blue, lwd = 2)
pi22.CI <- quantile(chain1.post.pi.k2[burnin:(niter-1),2], c(0.025, 0.975))
...
png(paste0(Sys.Date(), " TwoRegimeEstimatedSEIR_seed", seed, "_T", T+1, ".png"), 
    width = 2000, height = 1500, res = 200)

par(mfrow=c(2,2), mar=c(4, 4, 2, 2), mgp=c(2, 0.9, 0))
...
## Observed true regimes
png(paste0(Sys.Date(), " TwoRegimeEstimatedXt.png"), 
    width = 2000, height = 1500, res=200)
# par(mfrow=c(2,1))
# 
...
# Set the margins to reduce white space
...
plot(burnin:niter, marginalLogLik.CSMC.AS.repM[burnin:niter], type="l")
