...
f[2,1] <- 0.8                # Transmission rate modifier under regime 2
f[3,1] <- 0.5                 # Transmission rate modifier under regime 3
f[4,1] <- 0.1 
...
fromto.x4 <- seqToIntervals(which(x==4))
for (i in 1:nrow(fromto.x4)){
  rect(fromto.x4[i,1], -1, fromto.x4[i,2], 1, 
       col = rgb(0.5,0.3,0.3,1/4), border = rgb(0.5,0.3,0.3,1/4))
} 
...
saveRDS(sim_data, 
        file = paste0("~/Dropbox/Beta-Dirichlet-Time-Series-Model/pMCMC - BDSSSM/Data/Simulation Data/Four Regime/simulated_data_seed", seed, "_T", T+1,"_K",lenXset, ".RDS"))
...
# Read in data
sim_data <- readRDS(file = "~/Dropbox/Beta-Dirichlet-Time-Series-Model/pMCMC - BDSSSM/Data/Simulation Data/Four Regime/simulated_data_seed1700_T200_K4.RDS")
y <- sim_data$y
...
burnin <- 100
niter   <- 100 + burnin      # number of MCMC iterations
...
source("~/Dropbox/Beta-Dirichlet-Time-Series-Model/pMCMC - BDSSSM/Code/Four Regime/ParticleGibbs_FourRegimes.R")
...
setwd("~/Dropbox/Beta-Dirichlet-Time-Series-Model/pMCMC - BDSSSM/Output/Three Regime")
saveRDS(hyperparams, paste0("PG_results_hyperparams_seed", sim_data$seed, "_niter", niter,"_M", M, "_K", length(regimes), ".rds"))
saveRDS(PG.results, paste0("PG_results_seed", sim_data$seed, "_niter", niter, "_M", M, "_K", length(regimes), ".rds"))

# # Read results
# hyperparams <- readRDS("~/Dropbox/Beta-Dirichlet-Time-Series-Model/pMCMC - BDSSSM/Output/Simulation Study/Three Regime/2023-04-17 PG_results_hyperparams_seed3700_niter11000_M50.rds")
PG.results <- readRDS("~/Dropbox/Beta-Dirichlet-Time-Series-Model/pMCMC - BDSSSM/Output/Simulation Study/Three Regime/2023-04-17 PG_results_seed3700_niter11000_M50.rds")
...
############################## Data Visualization ##################################
sim_data <- readRDS("~/Dropbox/Beta-Dirichlet-Time-Series-Model/pMCMC - BDSSSM/Data/Simulation Data/Three Regime/simulated_data_seed3700_T150_K3.RDS")
seed <- sim_data$seed
...
pdf(paste0("~/Dropbox/Beta-Dirichlet-Time-Series-Model/pMCMC - BDSSSM/Figures/Simulation Study/Three Regime/",
           Sys.Date(), 
...
blue <- "#0072B2"
  ...
  pdf(paste0("~/Dropbox/Beta-Dirichlet-Time-Series-Model/pMCMC - BDSSSM/Figures/Simulation Study/Three Regime/", Sys.Date(), "ThreeRegimeEstimatedXt.pdf"), width = 10, height = 7)
  par(mfrow=c(2,1))
  
  plot(1:length(y), y, type = "p", lty=1, col = "grey", pch=20, main = "True Regimes", xlab = "Time")
  ...
  est.prob.x <- data.frame("prob.x1" = colSums(XsampleMat.CSMC.AS.repM == 1)/niter,
                           ...
                           "prob.x3" = colSums(XsampleMat.CSMC.AS.repM == 3)/niter)
