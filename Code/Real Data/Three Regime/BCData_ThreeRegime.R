...
hyperparams <- list(
  ...
  m.p2 = 0.3,
  sigma.p2 = 0.05,
  delta.mat = matrix(c(10, 1, 1, 
                       1, 10, 1, 
                       1, 1, 10), nrow = lenXset, ncol = lenXset),
  a.f = c(0.5, 0),
  b.f = c(1, 0.5)) 
...
# setwd("../../../Output/Real Data/Three Regime")
# saveRDS(hyperparams, paste0("PG_results_p2largerrange_hyperparams_BCWeekly_niter", niter,"_M", M, "_K", length(regimes), ".rds"))
# saveRDS(PG.results, paste0("PG_results_p2largerrange_BCWeekly_niter", niter, "_M", M, "_K", length(regimes), ".rds"))
# 
# str(PG.results)
...
plot(parameters.CSMC.AS.repM$p1[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(p[1]))
plot(parameters.CSMC.AS.repM$p2[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(p[2]))
