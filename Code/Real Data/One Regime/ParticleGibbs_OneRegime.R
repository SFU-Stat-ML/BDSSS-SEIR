...
PG.CSMC.AS <- function(y, regimes, M, niter, hyperparams, pop.size=1, num.mh.updates, T_star){
  ...
  # Set up matrices to store particles and parameters
  ...
  r = 1
  print(paste('PG-CSMC-AS with Rep M: iteration ', r))
  ...
  library("gtools")
  (parameters.CSMC.AS.repM$Px[[r]] <-  matrix(1))
  ...
  print(marginalLogLik.CSMC.AS.repM[r])
  ...
  while (r < niter) {
    ...
    mh.lambda.update <- update.lambda(
      y,
      RefParticleX,
      RefParticleS,
      RefParticleE,
    ...
    accept.p2 <- c(accept.p2, mh.p1.update$indicator)
    
    # pi.k in Px
    parameters.CSMC.AS.repM$Px[[r]] <- matrix(1)
    
    # f_{x_t}
    parameters.CSMC.AS.repM$f[1, r] <- matrix(1)
    }
  ...
  ## Acceptance rate
  accept.rate <- data.frame("Parameters"=c("alpha", "beta", "gamma", "lambda", "kappa", "p1",  "p2"),
