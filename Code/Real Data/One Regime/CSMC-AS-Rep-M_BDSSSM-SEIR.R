...
CSMC.AS.repM <- function(y,
  ...
  logWeights <- log.obs.density(y[1], particlesI[,1], lambda, p1)
  ...
  while (t <= T){
    ...
    ind[M] = m  # We don't resample the particle on the reference, which is saved as the last particle. check this. 
    
#     # Ancestor sampling step: Draw b_{t-1} (=a_t^(m))
#     ## hψ(y_t|theta_t^{(b_t)}, x_t^{(b_t)})
#     # log.hy <- dbeta(y[t], shape1 = lambda*p*particlesI[,t], shape2 = lambda*(1-p*particlesI[,t]), log = TRUE)      
#     if (t <= T_star){
#       log.hy <- dbeta(y[t], shape1 = lambda*p1*I_ref[t], shape2 = lambda*(1-p1*I_ref[t]), log = TRUE)      
#     }else{
#       log.hy <- dbeta(y[t], shape1 = lambda*p2*I_ref[t], shape2 = lambda*(1-p2*I_ref[t]), log = TRUE)      
#     }
# 
#     ## gψ(θ(Bt)t|θ(i)t−1, x(Bt)t)
...
# 
# #      log.gtheta[i,1] <- DirichletReg::ddirichlet(matrix(c(S_ref[t], E_ref[t], I_ref[t], R_ref[t]), nrow=1),
# #                                                  kappa * c(particlesS[i,t-1], particlesE[i,t-1],
# #                                                            particlesI[i,t-1], particlesR[i,t-1]), log = TRUE)
# 
#     }
#     
#     ## pψ(x(bt)t|x(i)t−1)
#     log.px <- log(Px[particlesX[,t-1], X_ref[t]])
#     # log.px <- log(Px[X_ref[t-1], X_ref[t]])
#     
#     ## hψ(y_t|theta_t^{(b_t)}, x_t^{(b_t)}) x gψ(θ(Bt)t|θ(i)t−1, x(Bt)t) x pψ(x(bt)t|x(i)t−1) x w^(bt−1)_(t-1)
#      log_w_as <- log(normalisedWeights[, t-1]) + log.hy + log.gtheta + log.px  # ?
#     
# #    log_w_as <- log.hy + log.gtheta + log.px 
...
# 
#     # ind0 <- sample(1:N, 1, prob = normalize_w_as)
#     # ind[M] <- m
#     # particlesS[m, t-1] <- particlesS[ind0, t-1]
#     # particlesE[m, t-1] <- particlesE[ind0, t-1]
#     # particlesI[m, t-1] <- particlesI[ind0, t-1]
#     # particlesR[m, t-1] <- particlesR[ind0, t-1]
#     # particlesX[m, t-1] <- particlesX[ind0, t-1]
#     
    ...
    if (t <= T_star){
      logWeights <- log.obs.density(y[t], particlesI[,t], lambda, p1) + log(target_transition_probs)
    }else{
      logWeights <- log.obs.density(y[t], particlesI[,t], lambda, p2) + log(target_transition_probs)
    }
    max_logWeight <- max(logWeights, na.rm = TRUE)
