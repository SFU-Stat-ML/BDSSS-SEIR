...
CSMC.AS.repM <- function(y,
  ...
  while (t <= T){
    ...
    log.hy <- dbeta(y[t], shape1 = lambda*p*I_ref[t], shape2 = lambda*(1-p*I_ref[t]), log = TRUE)      
    ...
    ## hψ(y_t|theta_t^{(b_t)}, x_t^{(b_t)}) x gψ(θt|θ(bt−1)t−1,x(bt)t) x pψ(x(bt)t|x(bt−1)t−1) x w^(bt−1)_(t-1)
