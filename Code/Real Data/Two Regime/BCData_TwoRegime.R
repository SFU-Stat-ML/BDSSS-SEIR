...
g.weekly <- ggplot(new_data_weekly, aes(x = Week, y = weekly.n)) +
  ...
  scale_x_date(limit = c(as.Date(min(bc_data_active$Date)), as.Date(max(bc_data_active$Date)))) +
  annotate("text", x = as.Date("2021-11-30"), y = max(bc_data_active$ActiveCases), 
           label = "", vjust = -20, hjust = 1.1,
           size = 5, color = "red")
g.weekly

# ggsave(file="../../../Figures/Real Data/BCWeeklyActiveCases.png", g.weekly, width = 7, height = 4)
...
burnin <- 1000
niter   <- 100000 + burnin      # number of MCMC iterations
...
PG.results <- foreach(i = 1:nchain, .combine = "list", .packages = c("truncnorm", "DirichletReg")) %dopar% {
  # Run PG sampler
  PG.CSMC.AS(y, regimes, M, niter, hyperparams, pop.size=1, num.mh.updates=5, T_star)
...
# setwd("../../../Output/Real Data/Two Regime")
# saveRDS(hyperparams, paste0("2024-01-06 PG_results_p2largerrange_hyperparams_niter", niter,"_M", M, "_K", length(regimes), ".rds"))
# saveRDS(PG.results, paste0("2024-01-06 PG_results_p2largerrange_niter", niter, "_M", M, "_K", length(regimes), ".rds"))
# 
# # Read results
# hyperparams <- readRDS("PG_results_p2largerrange_hyperparams_niter101000_M50_K2.rds")
# PG.results <- readRDS("PG_results_p2largerrange_niter101000_M50_K2.rds")
...
# ### Calculate marginal log likelihood for K=1, 3, 4
# 
# ### For one regime
# regimes <- c(1)
# burnin <- 1000
# niter <- 10000 + burnin
# source("../One Regime/ParticleGibbs_OneRegime.R")
# 
# ## Set up hyperparameters
# hyperparams <- list(
#   m.alpha = 0.2*7,
...
# 
# ptm <- proc.time()
# PG.results.K1 <- PG.CSMC.AS(y, regimes, M, niter, hyperparams, pop.size=1, num.mh.updates=5, T_star)
# proc.time()-ptm 
# 
# mean(PG.results.K1$marginalLogLik.CSMC.AS.repM[burnin:niter])
# 
# ### For three regimes
# regimes <- c(1,2,3)
# burnin <- 1000
# niter <- 10000 + burnin
# source("../Three Regime/ParticleGibbs_ThreeRegimes.R")
# 
# ## Set up hyperparameters
# hyperparams <- list(
#   m.alpha = 0.2*7,
...
#   m.p2 = 0.3,
#   sigma.p2 = 0.05,
#   delta.mat = matrix(c(10, 1, 1, 1, 10, 1, 1, 1, 10), nrow = 3, ncol = 3),
#   a.f = c(0.5, 0),
#   b.f = c(1, 0.5)) 
# 
# # Time of introducing antigen test
...
# 
# ptm <- proc.time()
# PG.results.K3 <- PG.CSMC.AS(y, regimes, M, niter, hyperparams, pop.size=1, num.mh.updates=5, T_star)
# proc.time()-ptm 
# 
# mean(PG.results.K3$marginalLogLik.CSMC.AS.repM[burnin:niter])
# 
# ### For four regimes
# regimes <- c(1,2,3,4)
# burnin <- 1000
# niter <- 10000 + burnin
# source("../Four Regime/ParticleGibbs_FourRegimes.R")
# 
# ## Set up hyperparameters
# hyperparams <- list(
#   m.alpha = 0.2*7,
...
# 
# ptm <- proc.time()
# PG.results.K4 <- PG.CSMC.AS(y, regimes, M, niter, hyperparams, pop.size=1, num.mh.updates=5, T_star)
# proc.time()-ptm 
# 
# mean(PG.results.K4$marginalLogLik.CSMC.AS.repM[burnin:niter])

############################ Data Visualization #######################################
setwd("../../../Figures/Real Data")

# Trace plot of parameters
png(paste("(Weekly)TwoRegimeTracePlot_TwoIdentificationRate.png"), width = 5000, height = 5000, res=500)
par(mfrow=c(5,4)) 
options(scipen = 1000) # Display large numbers without scientific notation

plot(MCMC.chain.1$parameters.CSMC.AS.repM$alpha[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(alpha), cex=2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$alpha[1, burnin:niter], col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$alpha[1, burnin:niter]), type="l", main="", xlab="", ylab = expression(alpha), ylim = c(0, 1))
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$alpha[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$beta[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(beta), cex=2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$beta[1, burnin:niter], col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$beta[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(beta))
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$beta[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$gamma[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(gamma), cex=2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$gamma[1, burnin:niter], col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$gamma[1, burnin:niter]), type="l",  main="",  xlab="", ylab = expression(gamma))
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$gamma[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$kappa[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(kappa), cex=2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$kappa[1, burnin:niter], col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$kappa[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(kappa))
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$kappa[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$lambda[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(lambda), cex=2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$lambda[1, burnin:niter], col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$lambda[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(lambda))
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$lambda[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$p1[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(p[1]))
lines(MCMC.chain.2$parameters.CSMC.AS.repM$p1[1, burnin:niter], col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$p1[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(p[1]))
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$p1[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$p2[1, burnin:niter], type="l", xlab="Iterations", ylab = expression(p[2]))
lines(MCMC.chain.2$parameters.CSMC.AS.repM$p2[1, burnin:niter], col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$p2[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(p[2]))
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$p2[1, burnin:niter]), col="blue")

plot(MCMC.chain.1$parameters.CSMC.AS.repM$f[2, burnin:niter], type="l", xlab="Iterations", ylab = expression(f[2]), cex=2)
lines(MCMC.chain.2$parameters.CSMC.AS.repM$f[2, burnin:niter], col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$f[2, burnin:niter]), type="l",  main="", xlab="", ylab = expression(f[2]), ylim=c(0, 3))
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$f[2, burnin:niter]), col="blue")


# Posterior Px
MCMC.chain.1.post.pi.k1 <- matrix(0, niter, lenXset, byrow=TRUE)
MCMC.chain.1.post.pi.k2 <- matrix(0, niter, lenXset, byrow=TRUE)
MCMC.chain.2.post.pi.k1 <- matrix(0, niter, lenXset, byrow=TRUE)
MCMC.chain.2.post.pi.k2 <- matrix(0, niter, lenXset, byrow=TRUE)
for (i in 1:(niter)){
  MCMC.chain.1.post.pi.k1[i-1,] <- MCMC.chain.1$parameters.CSMC.AS.repM$Px[[i]][1,]
  MCMC.chain.1.post.pi.k2[i-1,] <- MCMC.chain.1$parameters.CSMC.AS.repM$Px[[i]][2,]
  MCMC.chain.2.post.pi.k1[i-1,] <- MCMC.chain.2$parameters.CSMC.AS.repM$Px[[i]][1,]
  MCMC.chain.2.post.pi.k2[i-1,] <- MCMC.chain.2$parameters.CSMC.AS.repM$Px[[i]][2,]
}


# pi_{11}
plot(MCMC.chain.1.post.pi.k1[burnin:(niter-1),1], 
     type="l", 
     xlab="Iterations", 
     ylab = expression(pi[11]), cex=1.5) # pi_11
lines(MCMC.chain.2.post.pi.k1[burnin:(niter-1),1], col="blue")
plot(density(MCMC.chain.1.post.pi.k1[burnin:(niter-1),1]),
     type="l",
     xlab="Iterations",
     ylab = expression(pi[11]),
     main= "") # pi_11
lines(density(MCMC.chain.2.post.pi.k1[burnin:(niter-1),1]), col="blue")

# pi_{22}
plot(MCMC.chain.1.post.pi.k2[burnin:(niter-1),2], 
     type="l", 
     xlab="Iterations", 
     ylab = expression(pi[22]), cex=1.5)
lines(MCMC.chain.2.post.pi.k2[burnin:(niter-1),2], col="blue")
plot(density(MCMC.chain.1.post.pi.k2[burnin:(niter-1),2]),
     type="l",
     xlab="Iterations",
     ylab = expression(pi[22]),
     main = "")
lines(density(MCMC.chain.2.post.pi.k2[burnin:(niter-1),2]), col="blue")
...
niter <- 101000   # Update with your total number of iterations
...
chains_p1 <- mcmc.list(
  as.mcmc(MCMC.chain.1$parameters.CSMC.AS.repM$p1[, burnin:niter]),
  as.mcmc(MCMC.chain.2$parameters.CSMC.AS.repM$p1[, burnin:niter])
)

chains_p2 <- mcmc.list(
  as.mcmc(MCMC.chain.1$parameters.CSMC.AS.repM$p2[, burnin:niter]),
  as.mcmc(MCMC.chain.2$parameters.CSMC.AS.repM$p2[, burnin:niter])
)
...
chains_pi.k1 <- mcmc.list(
  as.mcmc(MCMC.chain.1.post.pi.k1[burnin:(niter-1),1]),
  as.mcmc(MCMC.chain.2.post.pi.k1[burnin:(niter-1),1])
)

chains_pi.k2 <- mcmc.list(
  as.mcmc(MCMC.chain.1.post.pi.k2[burnin:(niter-1),2]),
  as.mcmc(MCMC.chain.2.post.pi.k2[burnin:(niter-1),2])
)
...
gelman.diag(chains_p1)
gelman.diag(chains_p2)
gelman.diag(chains_f)
gelman.diag(chains_pi.k1)
gelman.diag(chains_pi.k2)


# Density plot of parameters from two chains
par(mfrow=c(3,3))
par(mar = c(5, 7, 4, 2) + 0.1)
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$alpha[1, burnin:niter]), type="l", main="", xlab="", ylab = expression(alpha), ylim = c(0, 1), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$alpha[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$beta[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(beta), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$beta[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$gamma[1, burnin:niter]), type="l",  main="",  xlab="", ylab = expression(gamma),cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$gamma[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$kappa[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(kappa), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$kappa[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$lambda[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(lambda),  cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$lambda[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$p1[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(p1), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$p1[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$p2[1, burnin:niter]), type="l",  main="", xlab="", ylab = expression(p2), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$p2[1, burnin:niter]), col="blue")
plot(density(MCMC.chain.1$parameters.CSMC.AS.repM$f[2, burnin:niter]), type="l",  main="", xlab="", ylab = expression(f[2]), ylim=c(0, 2.5), cex.lab=1.5)
lines(density(MCMC.chain.2$parameters.CSMC.AS.repM$f[2, burnin:niter]), col="blue")
...
png(paste("(Weekly)TwoRegimeHistogram_TwoIdentificationRates.png"), width = 5000, height = 3000, res=500)
par(mfrow=c(3,4))
...
# p1
hist(
    parameters.CSMC.AS.repM$p1[1,burnin:niter],
    xlab = expression(p[1]),
    main = "",
    col = "lightgrey",
    border = "white",
...
axis(1, col = "gray70")
axis(2, col = "gray70")
# abline(v = median(parameters.CSMC.AS.repM$p1[1,burnin:niter]), col = blue, lwd = 2)
abline(v = mean(parameters.CSMC.AS.repM$p1[1,burnin:niter]), col = blue, lwd = 2)
p1.CI <- quantile(parameters.CSMC.AS.repM$p1[1,1:niter], c(0.025, 0.975))
abline(v = p1.CI, col = blue, lty = 2, lwd = 2)

# p2
hist(
  parameters.CSMC.AS.repM$p2[1,burnin:niter],
  xlab = expression(p[2]),
  main = "",
  col = "lightgrey",
  border = "white",
...
axis(1, col = "gray70")
axis(2, col = "gray70")
# abline(v = median(parameters.CSMC.AS.repM$p2[1,burnin:niter]), col = blue, lwd = 2)
abline(v = mean(parameters.CSMC.AS.repM$p2[1,burnin:niter]), col = blue, lwd = 2)
p2.CI <- quantile(parameters.CSMC.AS.repM$p2[1,1:niter], c(0.025, 0.975))
abline(v = p2.CI, col = blue, lty = 2, lwd = 2)
...
# pi_11
hist(
    MCMC.chain.1.post.pi.k1[burnin:(niter-1),1],
...
abline(v = mean(MCMC.chain.1.post.pi.k1[burnin:(niter-1),1]), col = blue, lwd = 2)
pi11.CI <- quantile(MCMC.chain.1.post.pi.k1[burnin:(niter-1),1], c(0.025, 0.975))
abline(v = pi11.CI, col = blue, lty = 2, lwd = 2)
  
  
# pi_22
hist(
    MCMC.chain.1.post.pi.k2[burnin:(niter-1),2],
...
abline(v = mean(MCMC.chain.1.post.pi.k2[burnin:(niter-1),2]), col = blue, lwd = 2)
pi22.CI <- quantile(MCMC.chain.1.post.pi.k2[burnin:(niter-1),2], c(0.025, 0.975))
...
png(paste("(Weekly)TwoRegimeEstimatedSEIR_TwoIdentificationRate.png"), 
...
plot(as.Date(time), 
     colMeans(SsampleMat.CSMC.AS.repM[burnin:niter,]),
     xlab = "",
     type = "l",
     ylab = "Proportion",
     col = "#969696",
...
# Plot the mean trajectory
plot(as.Date(time), 
     colMeans(EsampleMat.CSMC.AS.repM[burnin:niter,]),
     xlab = "",
     type = "l",
     ylab = "Proportion",
     col = "#969696",
...
# Plot the mean trajectory
plot(as.Date(time), 
     colMeans(IsampleMat.CSMC.AS.repM[burnin:niter,]),
     xlab = "",
     type = "l",
     ylab = "Proportion",
     col = "#969696",
...
# Plot the mean trajectory
plot(as.Date(time), 
     colMeans(RsampleMat.CSMC.AS.repM[burnin:niter,]),
     xlab = "",
     type = "l",
     ylab = "Proportion",
     col = "#969696",
...
# Estimated X_t
png(filename = "(Weekly)TwoRegimeEstimatedXt_TwoIdentificationRates.png", width = 5000, height = 2500, units = "px", res = 500)
# pdf(paste("(Weekly)TwoRegimeEstimatedXt.pdf"), width = 10, height = 5)
...
estimated.y <- c(apply(IsampleMat.CSMC.AS.repM[burnin:niter,1:37]*mean(parameters.CSMC.AS.repM$p1[,burnin:niter]), 2, mean),
                 apply(IsampleMat.CSMC.AS.repM[burnin:niter,38:95]*mean(parameters.CSMC.AS.repM$p2[,burnin:niter]), 2, mean))
lines(as.Date(time), estimated.y, col="gray20")
...
# Compute the x and y coordinates for the shaded area
xx <- as.Date(time)
y_lower <- c(I.CredInterval.LL[1:37]*mean(parameters.CSMC.AS.repM$p1[,burnin:niter]), I.CredInterval.LL[38:95]*mean(parameters.CSMC.AS.repM$p2[,burnin:niter]))
y_upper <- c(I.CredInterval.UL[1:37]*mean(parameters.CSMC.AS.repM$p1[,burnin:niter]), I.CredInterval.UL[38:95]*mean(parameters.CSMC.AS.repM$p2[,burnin:niter]))
...
abline(v = as.Date("2020-3-27"), lty = 3, lwd=2, col = red, cex=1.5) # Distancing: Recommendation to avoid gatherings of any size issued
